...
let main = async function () {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    await ShellExec(`inkscape --export-area-page --export-dpi=300 --export-background=white --export-png="${path.resolve(dirname, filenameNoExt + '-tmp.png')}" "${file}"`)

    // console.log({file})

    await ShellExec(`inkscape --export-area-page --export-emf="${path.resolve(dirname, filenameNoExt + '.emf')}" "${file}"`)
